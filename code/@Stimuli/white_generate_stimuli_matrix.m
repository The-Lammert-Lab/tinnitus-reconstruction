function [stimuli_matrix, Fs, spect_matrix, binned_repr_matrix] = white_generate_stimuli_matrix(self)

    % generate first stimulus
    binned_repr_matrix = zeros(self.n_bins, self.n_trials);
    [stim1, Fs, spect, binned_repr_matrix(:, 1)] = self.white_generate_stimuli();
    ...
    for ii = 2:self.n_trials
        [stimuli_matrix(:, ii), ~, spect_matrix(:, ii), binned_repr_matrix(:, ii)] = self.white_generate_stimuli();
    end
