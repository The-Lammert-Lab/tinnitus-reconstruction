...
\begin{figure}[h] % Figure at bottom of the page ([b] argument, could be "t" for top or "h" for here)
	...
	Reverse correlation is used to reconstruct the latent internal representation
	from many trials.
...
\begin{figure}[h] % Figure at bottom of the page ([b] argument, could be "t" for top or "h" for here)
	...
	Tinnitus representation waveforms were generated using examples from the American Tinnitus Association website with added Gaussian noise.
	(\emph{a}) shows the frequency spectra for representative waveforms. In (\emph{b}), the waveforms
	are dimensonally-reduced using UMAP from 8,193 dimensions to 2.
	The waveforms cluster in this lower dimensional, revealing latent similarities.}
...
We enforce minimal constraints on the stimuli (\eg that the frequency content is audible).
