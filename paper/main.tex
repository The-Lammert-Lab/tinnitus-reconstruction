...
\author{Alec Hoyland, Nelson V. Barnett, Benjamin W. Roop, Danae Alexandrou, Myah Caplan, Jacob Mills, Ben Parell, Divya A. Chari, and Adam C. Lammert\textsuperscript{\textdagger{}}%
...
% NB SUGGESTION: However, existing strategies for characterizing the tinnitus sounds, such as Pitch Matching (PM) methods, are best suited for patients with tonal tinnitus, who experience sounds resembling pure tones (\eg ringing).
...
% "for who" is incorrect. Suggested sentence: There is a pressing need for characterization methods targeting tinnitus patients who are not served by current procedures
...
% INTRO PARAGRAPH 3/3
% NB SUGGESTION: Here, we validate RC as a method for characterizing all component
...
% "The door the developing is a typo. NB SUGGESTION: Our results demonstrate, for the first time, that tinnitus-like sounds with complex spectra can be accurately estimated using RC. 
...
% NB NOTE: I don't think b= is necessary. Can just be 8 bins.
% SUGGESTION: "the number of bins should increase with the level of detail in the frequency spectrum." 
% ALTERNATE: "the number of bins should increase with the level of detail desired in the reconstruction."
...
% NB NOTE: I'm not sure fig. 1 needs to be cited here, since it was above and it doesn't show tinnitus only exp.
(Fig. \ref{fig:experimentdiagram}).
...
% NB COMMENT: Should this be $x \in \mathbb{R}^\mathrm{t}$
...
% NB SUGGESTION: By adopting the $A-X$ paradigm for the present experiments,
% we have the opportunity to directly validate the reconstructions
% $\hat{x}_{buzzing}$ and $\hat{x}_{roaring}$ against their corresponding target sound.
...
where $Q(x, y)$ is the quantile function for $x \in [0, 1]$ of the similarity calculation $\Psi^\mathrm{T} s$.
...
% NB NOTE: I think it should be either "buzzing and roaring experiments' conditions" or "... experiment conditions"
...
% NB SUGGESTION: Break sentence here. Start with "This indicates..."
